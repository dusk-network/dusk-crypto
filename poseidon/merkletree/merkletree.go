...
// Perform a poseidon hash for a provided pair of bytes
func innerHash(a, b []byte) ([]byte, error) {
	p := poseidon.New()
...
func VerifyNode(n *Node) ([]byte, error) {
	...
	return innerHash(leftBytes, rightBytes)
}
...
func CalculateNodeHash(n *Node) ([]byte, error) {
	...
	return innerHash(n.Left.Hash, n.Right.Hash)
}
...
func create(pl []Payload) (*Node, []*Node, error) {
	...
	if len(leaves)%poseidon.MerkleArity == 1 {
...
func createIntermediate(nl []*Node) (*Node, error) {
	...
	for i := 0; i < len(nl); i += 2 {
		...
		h, err := innerHash(nl[left].Hash, nl[right].Hash)
		if err != nil {
			return nil, err
		}
	...
	for _, leaf := range t.Leaves {
		...
		if ok {
			...
			for currentParent != nil {
				...
				h, err := innerHash(leftBytes, rightBytes)
